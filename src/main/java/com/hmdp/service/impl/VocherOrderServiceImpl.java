        ...
        public void run() {
            while (true) {
                try {
                    ...
                    // 2.3 ACK确认:SACK stream.order g1 消息Id
                    stringRedisTemplate.opsForStream().acknowledge(queueName, "g1", record.getId());
                } catch (Exception e) {
                    // 出现异常则说明没有被ACK确认
                    // 此时需要从Pending-list中获取未确认的消息进行处理
                    handlePedingList();
                }
            }
        }

        private void handlePedingList() {
            while (true) {
                try {
                    ...
                    // 2.判断消息是否获取成功
                    // 2.1 失败则说明Pending-list没有异常消息，结束循环
                    if (list == null || list.isEmpty()) {
                        break;
                    }
                    // 2.2 成功
...
//    }
    ...
    // 将阻塞队列实现秒杀优化改为消息队列实现秒杀优化
    @Override
    public Result seckillVoucher(Long voucherId) {
        // 1.执行Lua脚本，判断有无抢购资格(为0即有购买资格)
        // 获取用户Id和订单Id(用于传入Lua脚本)
        Long userId = UserHolder.getUser().getId();
        long orderId = redisIdWorker.nextId("order");
        Long result = stringRedisTemplate.execute(
                SECKILL_SCRIPT,
        ...
        // Lua脚本中已经将消息添加到消息队列中
        // 2.初始化代理对象给另一个线程使用(这样保证另一个线程和主线程使用相同的代理对象)
        proxy = (IVoucherOrderService) AopContext.currentProxy();
